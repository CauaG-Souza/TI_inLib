# Protótipo de Alta Fidelidade
 
## 1. Acesso ao Protótipo
 
[link público para visualização do protótipo no Figma]
 
## 2. Identidade visual
 
**Cores principais:**
- Turquesa: destaque e ações principais.
- Roxo: identidade visual e modo escuro.
- Creme: fundo do modo claro.
 
**Tipografia:**
- Poppins, sans-serif.
 
## 3. Telas
 
### Tela 01 — Tela Inicial
 
**Wireframe relacionado:**

- Tela 01 — Tela inicial
 
**User Flows relacionados:**

- Fluxo 01 — Cadastro e acesso de professor
- Fluxo 02 — Cadastro e acesso de responsável
- Fluxo 03 — Busca e acesso aos materiais
 
**Histórias de Usuário relacionadas:**

- HU01 — Buscar materiais pedagógicos adaptados para estudantes com Síndrome de Down e TEA
- HU03 — Acesso aos materiais pelos responsáveis
- HU08 — Cadastro de professor
- HU09 — Cadastro de responsável 
- HU10 — Acesso à plataforma
 
**Protótipo:**

![Protótipo 01 — Tela inicial (Tema claro)](assets/img/protótipo_01_tela_inicial_claro.png)

![Protótipo 01 — Tela inicial (Tema escuro)](assets/img/protótipo_01_tela_inicial_escuro.png)
 
---
 
### Tela 02 — Login de professor

**Wireframe relacionado:**

- Tela 02 — Login de professor

**User Flow relacionado:**

- Fluxo 01 — Cadastro e acesso de professor

**História de Usuário relacionadas:**

- HU10 — Acesso à plataforma

**Protótipo:**

![Protótipo 02 — Acesso do professor (Tema claro)](assets/img/protótipo_02_acesso_do_professor_claro.png)

![Protótipo 02 — Acesso do professor (Tema escuro)](assets/img/protótipo_02_acesso_do_professor_escuro.png)

---

### Tela 03 — Cadastro de professor

**Wireframe relacionado:**

- Tela 03 — Cadastro de professor

**User Flow relacionado:**

- Fluxo 01 — Cadastro e acesso de professor

**História de Usuário relacionadas:**

- HU08 — Cadastro de professor

**Protótipo:**
 
![Protótipo 03 — Cadastro de professor (Tema claro)](assets/img/protótipo_03_cadastro_de_professor_claro.png)

![Protótipo 03 — Cadastro de professor (Tema escuro)](assets/img/protótipo_03_cadastro_de_professor_escuro.png)

---

## 4. Alterações em relação aos Wireframes
 
Foi acrescentada a opção de tema claro e escuro, para oferecer maior personalização e conforto visual. Na tela inicial, foi adicionada uma barra superior para facilitar a identificação e o acesso às opções da plataforma.